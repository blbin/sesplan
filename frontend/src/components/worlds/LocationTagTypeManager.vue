<template>
  <section class="location-tag-type-manager detail-section">
    <div class="section-header">
      <h3>Manage Location Tag Types</h3>
    ...
    <v-dialog v-model="showDialog" persistent max-width="500px">
      <v-card :loading="isSaving">
        <v-card-title>
          <span class="headline">{{ editingTagType ? 'Edit Tag Type' : 'Add Tag Type' }}</span>
        </v-card-title>
        <v-card-text>
          <v-text-field
            v-model="tagTypeName"
...
export default defineComponent({
  ...
  setup(props, { emit }) { // Získáme emit z kontextu
    ...
    const openEditDialog = (tagType: LocationTagType) => {
    ...
    const saveTagType = async () => {
      ...
      try {
        if (editingTagType.value) {
          // Update
          const updateData: LocationTagTypeUpdate = { name: tagTypeName.value };
          const updated = await locationTagTypeApi.updateLocationTagType(props.worldId, editingTagType.value.id, updateData);
          // Update in local list
          const index = availableTagTypes.value.findIndex(t => t.id === updated.id);
          if (index !== -1) availableTagTypes.value[index] = updated;
        } else {
          // Create
          const createData: LocationTagTypeCreate = { name: tagTypeName.value };
          const created = await locationTagTypeApi.createLocationTagType(props.worldId, createData);
    ...
    const confirmDelete = (tagType: LocationTagType) => {
    ...
    const executeDelete = async () => {
      ...
      deleteError.value = null;
      try {
        await locationTagTypeApi.deleteLocationTagType(props.worldId, tagTypeToDelete.value.id);
