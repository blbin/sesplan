...
"""
        ...
        #    generated_data_dict['user_id'] = current_user.id # REMOVED - Do not assign creator user_id by default
        ...
        if entity_type == 'character':
            pydantic_arg_name = 'character_in' # Argument for crud.create_character
        elif entity_type == 'location':
            pydantic_arg_name = 'location' # Correct argument for crud.create_location
        elif entity_type == 'organization':
             pydantic_arg_name = 'organization' # Assuming 'organization' for crud.create_organization
        elif entity_type == 'item':
             pydantic_arg_name = 'item' # Assuming 'item' for crud.create_item
        else:
             # Fallback might be dangerous, consider raising an error for unsupported types
             raise ValueError(f"Unsupported entity type for CRUD argument mapping: {entity_type}")
        ...
        # Add user_id specifically for create_character - needed for world membership check inside CRUD
        if entity_type == 'character':
